...
from taming.modules.performer.reversible import ReversibleSequence, SequentialSequence
...
class StyleEncoder(nn.Module):
    def __init__(self, resolution, patch_size, in_channels, out_features, transformercfg, dropout=0., pool='both'):
        super(StyleEncoder, self).__init__()
        assert resolution % patch_size == 0, 'resolution of images must be divided by patch size'
        assert pool in ['mean', 'max', 'both']
        patchs_len = (resolution // patch_size) ** 2
        token_dim = transformercfg['dim']
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=8, p2=8),
            nn.Linear(patchs_len * in_channels, token_dim),
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, token_dim))
        self.dropout = nn.Dropout(0.5)
        self.transformer = Performer(**transformercfg)
        self.pool = pool
        self.to_latent = nn.Identity()
        mlp_in_dim = token_dim if self.pool != 'both' else token_dim * 2
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(mlp_in_dim),
            nn.Linear(mlp_in_dim, out_features)
        )
...
if __name__ == '__main__':
    ...
    encoder = StyleEncoder(64, 8, 3, 256, transformercfg)
